...
func LoadENVConfig() (config EnvironmentConfig, err error) {
	...
	port, err := strconv.Atoi(os.Getenv("ACC_APP_PORT"))
	if err != nil {
	...
	config = EnvironmentConfig{
		Env: os.Getenv("ENV"),
		AppConfig: AppConfig{
			Name:           os.Getenv("ACC_APP_NAME"),
			Version:        os.Getenv("ACC_APP_VERSION"),
			Port:           port,
			MaxRequestTime: cast.ToInt(os.Getenv("ACC_APP_MAX_REQUEST_TIME")),
		},
		ACCDatabase: ACCDatabase{
